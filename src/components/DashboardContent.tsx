...
import { BoardWithLists, ListResponse, CardResponse, CardPriority, CardCreate } from "../types/workboard.types";
...
export function DashboardContent() {
  ...
  const createDefaultLists = async (boardId: string) => {
    ...
    for (const list of defaultLists) {
      await workboardService.createList(
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Find which list the card is being moved from and to
    ...
    // Determine destination list (over.id could be a list or a card)
    if (cardsByList[over.id as string]) {
      destListId = over.id as string;
    } else {
      // It's a card, find its list
      for (const [listId, cards] of Object.entries(cardsByList)) {
    ...
    setCardsByList(prev => {
      ...
      if (sourceListId === destListId) {
        ...
        newDestCards = overIndex === -1 ? [...destCards, movedCard] : [...destCards.slice(0, overIndex), movedCard, ...destCards.slice(overIndex)];
      }
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      >
        ...
        <DragOverlay>
          {draggedCardId && (() => {
            // Find the card being dragged from all lists
